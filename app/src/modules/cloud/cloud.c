...
static void handle_storage_batch_available(const struct storage_msg *msg)
{
	...
	uint16_t items_available = msg->data_len;
	...
	LOG_INF("Processing storage batch: %d items available", items_available);

	/* Process all available items in the batch */
	while (items_processed < items_available) {
		err = storage_batch_read(&item, K_SECONDS(5));
		if (err == -EAGAIN) {
	...
	LOG_INF("Processed %d/%d storage items", items_processed, items_available);
