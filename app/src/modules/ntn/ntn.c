...
static void lte_lc_evt_handler(const struct lte_lc_evt *const evt)
{
	...
	case LTE_LC_EVT_NW_REG_STATUS:
		if (evt->nw_reg_status == LTE_LC_NW_REG_UICC_FAIL) {
			...
			ntn_msg_publish(NTN_CONNECTION_FAILED);
...
static void state_ntn_exit(void *obj)
{
	...
	/* Compute next pass time using SGP4 */
	sat_prediction_pass_t next_pass;
	err = sat_prediction_get_next_pass(
	...
	LOG_INF("Max elevation: %.2f degrees", next_pass.max_elevation);
	...
	int64_t time_to_pass = next_pass.start_time_ms - current_time;
	if (time_to_pass < 0) {
		LOG_ERR("Next pass time is in the past");
		return;
	}

	/* Start GNSS timer to wake up 300 seconds before pass */
	int64_t gnss_timeout_value = time_to_pass - CONFIG_APP_NTN_TIMER_GNSS_VALUE_SECONDS * 1000;
	k_timer_start(&state->gnss_timer,
			K_MSEC(gnss_timeout_value),
			K_NO_WAIT);

	/* Start LTE timer to wake up 20 seconds before pass */
	int64_t ntn_timeout_value = time_to_pass - CONFIG_APP_NTN_TIMER_NTN_VALUE_SECONDS * 1000;
	k_timer_start(&state->ntn_timer,
			K_MSEC(ntn_timeout_value),
			K_NO_WAIT);

	LOG_INF("GNSS timer set to wake up in %lld ms", gnss_timeout_value);
	LOG_INF("NTN timer set to wake up in %lld ms", ntn_timeout_value);
}
