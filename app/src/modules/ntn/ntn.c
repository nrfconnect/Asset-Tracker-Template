...
static void update_cached_tle(struct ntn_state_object *state, const struct ntn_msg *msg)
{
	strncpy(state->tle_name, msg->tle_name, sizeof(state->tle_name) - 1);
	strncpy(state->tle_line1, msg->tle_line1, sizeof(state->tle_line1) - 1);
	strncpy(state->tle_line2, msg->tle_line2, sizeof(state->tle_line2) - 1);
...
static void load_tle_from_kconfig(struct ntn_state_object *state)
{
	...
	strncpy(state->tle_name, CONFIG_APP_NTN_TLE_NAME, sizeof(state->tle_name) - 1);
	strncpy(state->tle_line1, CONFIG_APP_NTN_TLE_LINE1, sizeof(state->tle_line1) - 1);
	strncpy(state->tle_line2, CONFIG_APP_NTN_TLE_LINE2, sizeof(state->tle_line2) - 1);

	state->tle_name[sizeof(state->tle_name) - 1] = '\0';
	state->tle_line1[sizeof(state->tle_line1) - 1] = '\0';
	state->tle_line2[sizeof(state->tle_line2) - 1] = '\0';

	if (state->tle_name[0] != '\0' && state->tle_line1[0] != '\0' && state->tle_line2[0] != '\0') {
		state->has_valid_tle = true;
		LOG_INF("Loaded TLE from Kconfig for %s", state->tle_name);
	} else {
...
static void state_running_entry(void *obj)
{
	...
	*/
	...
	if (!state->has_valid_tle) {
		LOG_WRN("Provide TLE via shell before running SGP4: att_ntn set_tle \"<name>\" \"<line1>\" \"<line2>\"");
	}
#endif
...
static enum smf_state_result state_running_run(void *obj)
{
	...
	if (state->chan == &NTN_CHAN) {
		...
		case NTN_SHELL_SET_TLE:
			update_cached_tle(state, msg);
			LOG_INF("Stored manual TLE:");
...
static void state_sgp4_entry(void *obj)
{
	...
	if (!state->has_valid_tle) {
		LOG_WRN("SGP4 requested without TLE data.");
	}
