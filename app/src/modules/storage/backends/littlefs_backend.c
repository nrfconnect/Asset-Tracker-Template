...
static int create_storage_file_path(const struct storage_data *type, int file_index, char *out_path)
{
	...
	ret = snprintf(out_path, MAX_PATH_LEN, "%s/%s_%d.bin", mountpoint->mnt_point, type->name,
		       file_index);
...
static int create_storage_header_file_path(const struct storage_data *type, char *out_path)
{
	...
	ret = snprintf(out_path, MAX_PATH_LEN, "%s/%s.header", mountpoint->mnt_point, type->name);
...
static int lfs_storage_clear(void)
{
	...
	char file_path[LFS_NAME_MAX + MAX_PATH_LEN];
	int ret;
	...
	while (true) {
		...
		if (entry.type == FS_DIR_ENTRY_FILE) {
			snprintf(file_path, sizeof(file_path), "%s/%s", mountpoint->mnt_point,
				 entry.name);
