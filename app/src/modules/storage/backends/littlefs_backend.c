...
#define MAX_PATH_LEN CONFIG_APP_STORAGE_LITTLEFS_MAX_NAME_LEN
 ...
 */
static int create_storage_file_path(const struct storage_data *type, char *out_path,
				     size_t max_len)
{
	int ret = 0;

	ret = snprintf(out_path, max_len, "%s/%s.bin", mountpoint.mnt_point, type->name);
 ...
 * @return int 0 on success, negative errno on failure
 */
static int update_storage_file_header(struct fs_file_t *file,
...
static int lfs_storage_init(void)
{
	...
	STRUCT_SECTION_FOREACH(storage_data, t) {
		...
		rc = create_storage_file_path(t, file_path, sizeof(file_path));
		if (rc < 0) {
			return rc;
		}
...
static int lfs_storage_store(const struct storage_data *type, const void *data, size_t size)
{
	...
	/* Open storage file */
	rc = create_storage_file_path(type, file_path, sizeof(file_path));
	if (rc < 0) {
		return rc;
	}
