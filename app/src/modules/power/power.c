...
static int uart_disable(void)
{
	...
	if (!device_is_ready(shell_uart_dev) || !device_is_ready(uart1_dev)) {
		LOG_ERR("Shell and UART devices are not ready");
		return -ENODEV;
	}
	...
	err = pm_device_action_run(shell_uart_dev, PM_DEVICE_ACTION_SUSPEND);
	if (err) {
...
static int uart_enable(void)
{
	...
	err = pm_device_action_run(shell_uart_dev, PM_DEVICE_ACTION_RESUME);
	if (err) {
		LOG_ERR("pm_device_action_run, error: %d", err);
		return err;
	}
