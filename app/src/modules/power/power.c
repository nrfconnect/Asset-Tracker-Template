...
static void timer_sample_start(uint32_t delay_sec)
{
	...
	err = k_work_reschedule(&timer_sample_work, K_SECONDS(delay_sec));
	if (err < 0) {
...
static int sample_and_process(struct power_state_object *state_object)
{
	...
	err = charger_read_sensors(state_object->charger, &state_object->voltage,
				   &state_object->percentage, &state_object->temperature,
...
static void state_running_entry(void *obj)
{
	...
	// TODO: Initialize fuel gauge, read initial values, etc.
...
static void state_idle_entry(void *obj)
{
	...
	float idle_current = CONFIG_APP_POWER_IDLE_CURRENT_UA / 1e6;
...
static enum smf_state_result state_idle_run(void *obj)
{
	...
	if (state_object->chan == &power_chan) {
		...
		if (power_msg->type == POWER_SET_ACTIVE) {
			LOG_DBG("POWER_SET_ACTIVE received, transitioning to active state");
			smf_set_state(SMF_CTX(state_object), &states[STATE_ACTIVE]);

			return SMF_EVENT_HANDLED;
		}

		if (power_msg->type == POWER_SET_IDLE) {
			LOG_DBG("POWER_SET_IDLE received, already in idle state, ignoring");
			return SMF_EVENT_HANDLED;
		}
	}
...
static void state_active_entry(void *obj)
{
	...
	timer_sample_start(CONFIG_APP_POWER_SAMPLE_INTERVAL_SECONDS);
}
...
static enum smf_state_result state_active_run(void *obj)
{
	...
	if (state_object->chan == &power_chan) {
		...
		if (power_msg->type == POWER_SET_IDLE) {
			LOG_DBG("POWER_SET_IDLE received, transitioning to idle state");
		...
		if (power_msg->type == POWER_SET_ACTIVE) {
			LOG_DBG("POWER_SET_ACTIVE received, already in active state, ignoring");
			return SMF_EVENT_HANDLED;
		}
	}
