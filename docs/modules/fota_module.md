...
With the default configuration, which retrieves updates through the unified nRF Cloud backend (`CONFIG_MEMFAULT_ZEPHYR_FOTA_BACKEND_NRF_CLOUD_OVERRIDE`), the module supports two firmware image types:
...
To perform a delta modem update, see https://docs.nrfcloud.com/docs/mcu/nrf-modem-fota.

Once a download has started, the module publishes `FOTA_STARTING` so the application can move into its FOTA state. Application and delta modem images are marked ready after the download completes.

When the module needs the network to be disconnected (for example, to release modem resources before reboot), it publishes `FOTA_NETWORK_DISCONNECT_NEEDED`. The application must then:
