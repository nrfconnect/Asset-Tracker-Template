...
```
git cherry-pick docs/patches/magnetometer.patch
```
...
struct environmental_state {
	...
	/* Magnetic field measurements (X, Y, Z) in microtesla (µT) */
...
**REMOVE** Create a script and/or patch file that allows the user to add a new module to the template and is able to specify certain paramters of the module. Use code generation, update the docs on use, and refer to the command in the docs (main page?)
