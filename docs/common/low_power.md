...
CONFIG_APP_CLOUD_SYNC_INTERVAL_SECONDS=600              # Default: 10 minutes
```

In buffer mode, sensors and location are sampled and stored at every interval set in the `CONFIG_APP_BUFFER_MODE_SAMPLING_INTERVAL_SECONDS` Kconfig option and sent to the cloud at every interval as set in the `CONFIG_APP_CLOUD_SYNC_INTERVAL_SECONDS` Kconfig option.
