...
/* Test cloud location request with 0 Wi-Fi APs is rejected */
void test_cloud_location_request_zero_wifi_aps_rejected(void)
{
	struct wifi_scan_result mock_wifi_aps[1]; /* Array exists but cnt=0 */
	struct wifi_scan_info mock_wifi_info = {
		.ap_info = mock_wifi_aps,
		.cnt = 0  /* 0 access points */
	};
	struct location_data_cloud mock_cloud_request = {
		.cell_data = NULL,
		.wifi_data = &mock_wifi_info
	...
	/* Simulate cloud location request with 0 Wi-Fi APs */
	simulate_location_event(&mock_event);

	wait_for_processing();

	/* Verify that error result was set to allow fallback */
	TEST_ASSERT_EQUAL(1, location_cloud_location_ext_result_set_fake.call_count);
	TEST_ASSERT_EQUAL(LOCATION_EXT_RESULT_ERROR,
			  location_cloud_location_ext_result_set_fake.arg0_val);

	/* Verify that no LOCATION_CLOUD_REQUEST message was published */
	const struct zbus_channel *chan;
	struct location_msg received_msg;
	int err = zbus_sub_wait_msg(&test_subscriber, &chan, &received_msg, K_MSEC(100));

	/* Should timeout as no message should be sent */
	TEST_ASSERT_EQUAL(-ENOMSG, err);
}

/* Test cloud location request with NULL Wi-Fi and cellular data is rejected */
void test_cloud_location_request_no_data_rejected(void)
{
	struct location_data_cloud mock_cloud_request = {
		.cell_data = NULL,
		.wifi_data = NULL
	...
	/* Simulate cloud location request with no data */
	simulate_location_event(&mock_event);
...
/* Test cloud location request with cellular data and 0 Wi-Fi APs uses only cellular */
void test_cloud_location_request_cellular_with_zero_wifi(void)
{
	struct lte_lc_cell mock_cell = {
		.mcc = 242,
		.mnc = 1,
	...
	struct wifi_scan_result mock_wifi_aps[1];
	struct wifi_scan_info mock_wifi_info = {
		.ap_info = mock_wifi_aps,
		.cnt = 0  /* 0 access points */
	};
	...
	/* Simulate cloud location request with cellular + 0 Wi-Fi APs */
	simulate_location_event(&mock_event);

	wait_for_processing();

	/* Verify cloud location request message was published with only cellular data */
	verify_cellular_cloud_request(&mock_cells_info);
}

/* Test cloud location request with valid Wi-Fi data (cnt > 0) is accepted */
void test_cloud_location_request_valid_wifi_accepted(void)
{
	struct wifi_scan_result mock_wifi_aps[2] = {
		{
			.ssid_length = 7,
	...
	/* Simulate cloud location request with valid Wi-Fi data */
	simulate_location_event(&mock_event);
