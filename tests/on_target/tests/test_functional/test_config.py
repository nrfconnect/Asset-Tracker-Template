...
def test_config(dut_cloud, hex_file):
    '''
    Test that verifies shadow changes are applied c2d and d2c for all config parameters.
    Tests update_interval, sample_interval, and buffer_mode.
    '''
    ...
    dut_cloud.cloud.patch_config(dut_cloud.device_id, 
    ...
    try:
        while time.time() - start < CLOUD_TIMEOUT:
            ...
            if (update_interval == TEST_UPDATE_INTERVAL and 
                ...
                logger.debug(f"Config not yet fully updated. Current: update_interval={update_interval}, sample_interval={sample_interval}, buffer_mode={buffer_mode}")
                continue
        else:
            raise RuntimeError(f"Configuration not correctly reported to cloud. Expected: update_interval={TEST_UPDATE_INTERVAL}, sample_interval={TEST_SAMPLE_INTERVAL}, buffer_mode={TEST_BUFFER_MODE}. Got: update_interval={update_interval}, sample_interval={sample_interval}, buffer_mode={buffer_mode}")
        
        # Wait for config changes to be logged by the device
        dut_cloud.uart.wait_for_str(f"Updating update interval to {TEST_UPDATE_INTERVAL} seconds", timeout=120)
        dut_cloud.uart.wait_for_str(f"Updating sample interval to {TEST_SAMPLE_INTERVAL} seconds", timeout=120)
